...
The game state, start time (and thus time remaining) and player/team state is synchronized, as well as the [callbacks](#callback-system). All players (including players in the instance but not in the lobby) should see the same basic information about your game.

:::caution
Any other state for your game needs to be synchronized yourself. See the [Networking guide](/guides/networking) for more info on how to deal with this, as well as [VRChat's own Networking docs](https://creators.vrchat.com/worlds/udon/networking/).
:::
...
Mugi can call public methods on arbitrary UdonBehaviors at different points of the game lifecycle. To register callbacks, add your UdonBehavior to the appropriate array field on the MugiGame component, e.g. by dragging the gameobject into the inspector. These callbacks run on all clients (i.e. they are synchronized across the network).

:::tip
If you just want some gameobjects to only be enabled while the game is running, see [Lifecycle GameObjects](#lifecycle-gameobjects).
:::
...
:::note
Callbacks are called only once, when the game transitions between states. E.g. if a player joins an instance halfway through a game, they won't have OnMugiStart called. If you need this sort of "catchup" behavior, you may have to poll the MugiGame state yourself.
:::
...
Mugi can also automatically enable and disable GameObjects when the game is in the `Running` state. Add your gameobjects to the `Enable During Game` array on the `MugiGame` behavior. The gameobjects will be enabled for all players, including those outside the game.
