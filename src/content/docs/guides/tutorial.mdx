...
3. Click the "Add to VCC" button, which should open VCC (VRChat Creator Companion).
4. Add the extracted template folder as a project.
5. Click the 'Manage Project' Button on the new project.
...
Notice that there aren't any Scenes in the Assets/ folder, but there is in the `Packages/com.example.jamgame/Samples` folder.
...
```csharp title=CapsuleClicker.cs ins={16-27}
using UdonSharp;
using UnityEngine;
using VRC.SDKBase;
